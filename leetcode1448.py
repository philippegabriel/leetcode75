...
class Solution:
    ...
    def good_nodes_max(self, root: Optional[TreeNode], max_node: int=-10001) -> int:
        if not root:
            return 0
        acc = 0
        new_max = max_node
        if root.val and root.val >= max_node:
            acc = 1
            new_max = root.val
        return  acc \
...
def test() -> None:
    ...
    root = build_tree([1])
    print(s.goodNodes(root))
    root = build_tree([-1,5,-2,4,4,2,-2,None,None,-4,None,-2,3,None,
                       -2,0,None,-1,None,-3,None,-4,-3,3,None,None,None,None,None,None,None,
                       3,-3])
    print(s.goodNodes(root))
